function [b,a] = pass_Mag_Resp(N,Wn,varargin)
...
%   %   0.35 <= w <= 0.65.
%
%   b = pass_Mag_Resp(48,[0.35 0.65]);   % Window-based FIR filter design
...
%   b = pass_Mag_Resp(34,0.48,'high',chebwin(35,30));    % FIR filter design
%   freqz(b,1,512);                 % Frequency response of filter
...
[N,msg1,msg2,msgobj] = Fil_order_Validatoin_Fun(N,ff(end),aa,hilbert);
...
if isempty(indx),
    error(message('signal:pass_Mag_Resp:UnknFilterType'));
else
